...
def add_to_wishlist(request, id):
    ...
    if product.users_wishlist.filter(id=request.user.id).exists():
        ...
        messages.success(request, "已加入收藏夹")
    else:
        product.users_wishlist.add(request.user)
        messages.success(request, "已添加 " + product.title + " 到你的收藏夹")
...
def remove_from_wishlist(request, id):
    ...
    if product.users_wishlist.filter(id=request.user.id).exists():
        product.users_wishlist.remove(request.user)
        messages.success(request, "已将 " + product.title + " 移出你的收藏夹")
    return HttpResponseRedirect(request.META["HTTP_REFERER"])
...
def add_address_tmp(request):
    if request.method == "POST":
        ...
        if address_form.is_valid():
            ...
            return HttpResponse("Error handler tmp content", status=400)
    else:
        address_form = UserAddressForm()
    return render(
        request, "account/dashboard/edit_addresses_tmp.html", {"form": address_form}
    )
...
def add_address(request):
    if request.method == "POST":
        ...
        if address_form.is_valid():
            ...
            return HttpResponse("Error handler2 content", status=400)
    else:
        address_form = UserAddressForm()
    return render(
    ...
    if request.method == "POST":
        ...
        if product_add_form.is_valid() and product_image_add_form.is_valid():
            ...
            form = {
                ...
                "form_image": product_image_add_form,
                "update": 0,
            }
...
def ProductUpdate(request, pid):
    ...
    if request.method == "POST":
        ...
        product_image_add_form = ProductImageUpdateForm(
            data=request.POST, files=request.FILES
        )
        if product_add_form.is_valid() and product_image_add_form.is_valid():
            old_product.delete()
        ...
        product_add_form = ProductUpdateForm(
            initial=dict(old_product.__dict__, **{"category": old_product.category})
        )
        if old_image:
            product_image_add_form = ProductImageUpdateForm(initial=old_image.__dict__)
        else:
            product_image_add_form = ProductImageUpdateForm()
        form = {
            "form": product_add_form,
...
def AllOrder(request):
    products = (
        Product.objects.prefetch_related("order_product")
        .filter(is_active=True)
        .filter(seller=request.user.id)
    )
    print(products)
    return render(
        request, "account/dashboard/manage/all_order.html", {"products": products}
    )
...
def ItemOrder(request, pid):
    ...
    return render(
        request, "account/dashboard/manage/item_order.html", {"product": product}
    )
...
def view_seller(request, seller_id):
    ...
    return render(
        request,
        "account/dashboard/view_details.html",
        {"seller": seller[0], "products": products},
    )
